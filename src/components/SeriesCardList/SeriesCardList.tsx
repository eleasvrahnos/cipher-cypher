...
const SeriesCardList: React.FC<SeriesCardListProps> = ({
  ...
  const seriesInfo: Record<number, { title: string, description: string, difficultyRating: string, releaseDate: string, current: number, total: number, }> = {
    0: { title: "Math Mania", description: "For those with an interest in math and puzzles, this is the crossover you've always wanted! See if you can math it out to beat these puzzles.", difficultyRating: "/difficulty/mathmania.png", releaseDate: "April 11, 2020", current: puzzleStatuses[0].filter(element => element === true).length, total: 9,  },
    1: { title: "Puzzle Paradise", description: "For those who have a good foundational understanding of ciphers and critical thinking. Are you able to decode all of these puzzles?", difficultyRating: "/difficulty/puzzleparadise.png", releaseDate: "April 12, 2022", current: puzzleStatuses[1].filter(element => element === true).length, total: 18, },
    2: { title: "Riddling Rewind", description: "For those who want to challenge their cipher and cryptography knowledge. Thinking outside the box is required to complete this brutal set of puzzles.", difficultyRating: "/difficulty/riddlingrewind.png", releaseDate: "August 22, 2022", current: puzzleStatuses[2].filter(element => element === true).length, total: 23, },
  };
